...
DEFAULT_QUESTION = "What should I do if the platform is unavailable near an assignment deadline?"


def run_pipeline(base_dir: str | Path, question: str = DEFAULT_QUESTION) -> dict:
    base_path = Path(base_dir)
    dataset = ensure_corpus(base_path)
    response = build_support_answer(base_path, question=question, top_k=3)

    processed_dir = base_path / "data" / "processed"
    processed_dir.mkdir(parents=True, exist_ok=True)
    report_path = processed_dir / "student_support_copilot_report.json"
